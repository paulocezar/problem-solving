...
vector<pos_t> getPawnNeighbors(Player cur, pos_t pos, state_t &board) {
  vector<pos_t> ng;
  ...
  pos.first += dx;
  if (inside(pos) && (board[pos.first][pos.second] == EMPTY)) {
    ng.push_back(pos);
  }
...
vector<pos_t> getKnightNeighbors(pos_t pos, state_t &board) {
  vector<pos_t> captures;
  vector<pos_t> ng;
  ...
  for (auto &dir : moves[KNIGHT]) {
    ...
    if (board[npos.first][npos.second] == EMPTY) {
      ...
      if (isupper(board[npos.first][npos.second]) != isupper(board[pos.first][pos.second])) {
        captures.push_back(npos);
      }
    }
  }

  shuffle(ng.begin(), ng.end(), eng);
  captures.insert(captures.end(), ng.begin(), ng.end());
  return captures;
}

vector<pos_t> getNeighbors(pos_t pos, vector<dir_t> &dirs, state_t &board) {
  vector<pos_t> captures;
  vector<pos_t> ng;
...
Player minimax(Player cur, state_t board, int remaining_moves) {
  if (remaining_moves == 0) return BLACK;
  ...
  for (auto &p : pieces) {
    ...
    switch(toupper(piece)) {
      case PAWN:
        neighbors = getPawnNeighbors(cur, p, board);
        break;
      case KNIGHT:
        neighbors = getKnightNeighbors(p, board);
        break;
      case BISHOP:
      case ROOK:
      case QUEEN:
        neighbors = getNeighbors(p, moves[toupper(piece)], board);
        break;
    }
    ...
    for (auto &pos : neighbors) {
      ...
      for (Piece pp : possibilites) {
        ...
        if (minimax(1 - cur, board, remaining_moves - 1) == cur) {
          return cur;
        }
      }
...
int main(int argc, char* argv[]) {
  ...
  while (g--) {
    ...
    Player winner = minimax(WHITE, board, m);

    if (winner == WHITE) cout << "YES\n";
    else cout << "NO\n";
  }
