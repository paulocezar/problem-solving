...
const int ALPH = 1 << 16;
const int LGMSZ = 18;
const int MAXSZ = 1 << LGMSZ;
const int MAXN = 1 << 16;

int s[MAXSZ], id[MAXSZ];
vector<int> occ[MAXN];
...
vector<int> k_rec(const vector<int>& v, int k) {
  ...
  while (prec >= 0 && p0 >= 0)
    if (rec[prec]%3 == 1 && k_cmp(v[m0[p0]], v[rec[prec]], sub[m0[p0]/3], sub[rec[prec]/3+sz]) ||
        rec[prec]%3 == 2 && k_cmp(v[m0[p0]], v[rec[prec]], v[m0[p0]+1], v[rec[prec]+1], sub[m0[p0]/3+sz], sub[rec[prec]/3+1]))
      rec[pret--] = rec[prec--];
    else
      rec[pret--] = m0[p0--];
...
vector<int> karkkainen(int n) {
  int cnt = 1;
  vector<int> v(n + 3);

  for (int i = 0; i < n; ++i) v[i] = i;
  k_radix(s, &v[0], tmp, 0, n, ALPH);
...
int main(int argc, char* argv[]) {
  ...
  const auto &sa = karkkainen(sz);
  const auto &lc = lcp(sa);
