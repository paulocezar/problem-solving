...
char text[MAXN], seen[MAXM];
int N, M, sa[MAXN], rank[MAXN], LCP[MAXN], rel[MAXN];
...
bool find( int size, bool print = false ){
	...
	REP( beg, N ){
		...
		if( beg == N ) break;
		int end = beg;
		memset( seen, 0, sizeof(char)*M );
		seen[ rel[ sa[beg-1] ] ] = 1;
		int tseen = 1;
		while( end < N && LCP[end] >= size ){
			if( !seen[ rel[ sa[end] ] ] )
				tseen++; seen[ rel[ sa[end] ] ] = 1;
			end++;
		}
		if( tseen > M/2 ){
			if( print ){
...
int main( int argc, char* argv[] ){
	...
	while( cin >> M && M ){
		...
		REP( i, M ){
			cin >> aux;
			REP( j, SIZE(aux) ){ text[ N ] = aux[j]; rel[ N++ ] = i; }
			text[ N++ ] = 1000 + i;
		}
		if( M == 1 ){
			REP( i, N-1 ) cout << text[i]; cout << "\n"; 
			continue;
		}
		buildSA();
		buildLCP();
