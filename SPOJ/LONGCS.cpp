...
int N, SA[MAXN], LCP[MAXN], rank[MAXN], who[MAXN], bucket[CHAR_MAX-CHAR_MIN+1];
char bh[MAXN+1];
...
int main( int argc, char* argv[] ){
	...
	while(t--){
		...
		REP( j, k ){
			gets(x);
			int i = 0;
			while( x[i] ){ S[N] = x[i++]; who[N++] = j; }
			S[N] = char('A'+j); who[N++] = j;
		}
		S[N] = '\0';
		buildSA();
		...
		while( lo <= hi ){
			...
			while( from < N ){
				...
				if( LCP[from] >= mid ){
					...
					while( from < N && LCP[from] >= mid ) has |= 1<<who[SA[from++]];
