...
public:
	string message( vector <string> part, vector <string> dict, vector <string> keystr ) {
		...
		REP( i, SIZE(part) )
			REP( j, SIZE(part[i]) )
				cr[ part[i][j] ] = char('1'+i);
		
		map< string, vector<string> > rDict;
		REP( i, SIZE(dict) ){
			string now = "";
			REP( j, SIZE(dict[i]) )
				now += cr[ dict[i][j] ];
		...
		FORIT( ntry, rDict ) sort( ALL(ntry->second) );
		rDict[""].PB( "" );
		
		string msg = "";
		REP( i, SIZE(keystr) ) msg += keystr[i];
		string ans = "";
		string now = "";
		REP( i, SIZE(msg) ){
			if( '1' <= msg[i] && msg[i] <= '9' ){
