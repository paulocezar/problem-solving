...
public:
	int crack( vector <string> plaintexts, vector <string> ciphertexts ) {
		int M = SIZE(plaintexts);
		int N = SIZE(ciphertexts);
		int K = SIZE(plaintexts[0]);
		map< string, int > ct;
		REP( i, N ){
			REP( j, M ){
				string cip = "";
				REP( k, K ){
					if( ciphertexts[i][k] == plaintexts[j][k] ) cip += "0";
					else cip += "1";
				}
				ct[ cip ]++;
			}
...
namespace moj_harness {
	...
	int run_test_case(int casenum) {
      ...
      case 4: {
			string plaintexts[]       = {"11", "01", "00"};
			string ciphertexts[]      = {"00", "10", "11"};
			int expected__            = 1;
