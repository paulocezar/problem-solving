...
public:
    double getAverage( vector <string> rr ) {
        string s = "";
        REP(i,SIZE(rr)) s += rr[i];
        ...
        if (lst != n) {
            ...
            if (lst <= ellys) {
                res = ((double(sum) / double(R)) + ratings[ellys]) / double(grpsz + 1);
            } else {
                
                
                double pmr = double(n%R) / double(R);
                double pmn = 1.0 - pmr;
                
                res = pmn * (((double(sum) / double(R)) + ratings[ellys]) / double(grpsz));
                
                int sum2 = 0;
                FOR(i,lst,n-1) sum2 += ratings[i];
                
                res += pmr * (((double(sum)/double(R)) + (double(sum2)/double(n%R)) + ratings[ellys]) / double(grpsz+1));
                
            }
            
        } else {
            res = ((double(sum) / double(R)) + ratings[ellys]) / double(grpsz);
        }
