...
int solve(int at, int need) {
	if (!need) return 0;
	if (at >= (N-1)) return 2e9;

	int &ret = dp[at][need];
	if (cached[at][need]) return ret;
	cached[at][need] = 1;

	ret = solve(at+1, need);
	ret = min(ret, max(S[at+1]-S[at], solve(at+2, need-1)));
	return ret;
}

class EllysSocks {
public:
    int getDifference( vector <int> _S, int P ) {
    		S = _S;
        sort(S.begin(), S.end());
        N = S.size();
