...
public:
	int theMin( vector <string> switches, vector <string> lamps ) {
		...
		map< int64, int64 > prev;
		
		int N = SIZE(switches);
		int nlamps = SIZE(switches[0]);
		
		int64 maybe[nlamps];
		REP( i, nlamps ) maybe[i] = (1LL<<nlamps)-1LL;
		
		REP( i, N ){
			
			int64 swt = 0;
			REP( bit, nlamps ) if( switches[i][bit] == 'Y' )
				swt |= (1LL<<bit);
			
			int64 lmp = 0;
			REP( bit, nlamps ) if( lamps[i][bit] == 'Y' )
				lmp |= (1LL<<bit);
			
			if( prev.count(swt) && prev[swt] != lmp ) return -1;
			prev[swt] = lmp;
			...
			REP( swtch, nlamps ){
				if( swt & (1LL<<swtch) ){
					int64 now = 0;
					REP( lmpd, nlamps ) if( lmp & (1LL<<lmpd) )
						now |= (1LL<<lmpd);
					maybe[swtch] &= now;
				} else {
					int64 now = 0;
		...
		REP( swtch, nlamps ){
			...
			int onthis = bitson(maybe[swtch]);
			if( onthis == 1 ){
				if( !unq.insert( maybe[swtch] ).second ) return -1;
			} else ans = max( ans, onthis );
			
		}
		
		int rans = 0;
		while( (1<<rans) < ans ) rans++;
