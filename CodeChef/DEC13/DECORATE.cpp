...
#define MAXN 202202
#define BASE 33ULL
#define VALUE(c) ((c=='#')?(26):((c)-'a'))

typedef unsigned long long hash_t;

hash_t h[MAXN], pw[MAXN];
int ans[MAXN];

hash_t calc_hash(int beg, int end) {
...
int manacher(string &ss) {
    ...
    set< pair<int,hash_t> > seen;
    int cur = 1;
	while (cur < n) {
        
        while ((cur > ans[cur])
            && (cur+ans[cur]+1 < n)
            && (s[cur-ans[cur]-1] == s[cur+ans[cur]+1])) {
                int len = (cur+ans[cur]+1) - (cur-ans[cur]-1) + 1;
                
                if (s[cur-ans[cur]-1] != '#') {
                    seen.insert(make_pair(len, calc_hash(cur-ans[cur]-1,cur+ans[cur]+2)));
                } else {
                    seen.insert(make_pair(len-2, calc_hash(cur-ans[cur],cur+ans[cur]+1)));                    
                }
...
void sieve() {
    for (int i = 1; i <= 600; ++i) bs[i] = i&1;
    bs[1] = false;
    bs[2] = true;
    
    for (int i = 3; i <= 25; i += 2) if (bs[i]) {
        for (int j = i*i; j <= 600; j += i) bs[j] = false;
    }
   
    primes.push_back(2);
    for (int i = 3; i <= 600; i+=2) if (bs[i]) primes.push_back(i);
}
...
BigInt necklaces(int n, BigInt k) {
    BigInt res = 0;
    for (int i = 1; i*i <= n; ++i) {
        if (n % i) continue;
        int d = i;
        res += BigInt(phi(d)) * pow(k, n/d);
...
BigInt bracelets(int n, BigInt k) {
    if (n & 1) return (necklaces(n, k) + pow(k,(n+1)/2)) / BigInt(2);
    return (necklaces(n, k)/BigInt(2)) + (((k+BigInt(1))*pow(k,n/2))/BigInt(4));
}
